...
async function get(res) {
  ...
  const feed = await parser.parseURL("https://elfsborg.se/feed/"); // https://www.reddit.com/.rss
  ...
  await Promise.all(
    feed.items.map(async (currentItem) => {
      ...
      if(typeof currentItem.enclosure != "undefined"){
        currentItem.image = currentItem.enclosure.url;
        delete currentItem.enclosure;
      }
